...
static inline float my_fadd(float a, float b) {
	...
	asm volatile (
			".dc.w	0xfe5b\n"		/* 0xfe5b = __FADD */
	:	/* 出力 */	"=d" (reg_d0),	/* out %0 */	/* FE call 0xfe5b は d0/d1 に結果を返す */
					"=d" (reg_d1)	/* out %1 */	/* FE call 0xfe5b は d0/d1 に結果を返す */
	:	/* 入力 */	"irm" (reg_d0),	/* in %2 */		/* FE call 0xfe5b は d0.l を被加算数として受け取る */
					"irm" (reg_d1)	/* in %3 */		/* FE call 0xfe5b は d1.l を加算数として受け取る */
	);
	return reg_d0;
}
