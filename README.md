...
また、windows から利用可能な専用の名前付きパイプを利用することで、
エミュレーション中の X68K に任意のコマンドを実行させたり停止させることができます。
XEiJ を利用したデバッグは、
一般的なクロス開発環境におけるリモート実行を模したような動作となります。
X68K のプログラムからは、デバッグ用の printf ログ等を X68K のメイン画面ではなく独立したターミナルウィンドウ上に出力可能です。


## XEiJ と連携したデバッグ実行を行うための環境設定
...
6. xdev68k 上でビルド＆デバッグ実行＆中断  
	...
	XEiJ を起動し、X68K 側がコマンドラインの受付け状態になっていることを確認した上で、
...
VSYNC やタイマー割り込みなどを利用したプログラムのデバッグを行う場合は注意が必要です。
デバッグ中にプログラムを中断した場合、割り込み処理を停止するための後処理が行われないままプログラムが終了します。
現状では、この状態に陥ると復帰する手段は無く、X68K 側を再起動する必要があります。

（ややアクロバチックですが、
デバッグ開始前のハードウェアステートを退避し、
デバッグ終了時に復帰するような仕組みを作ることができれば解決するかもしれません。
将来の課題です。）
...
* 大きなメモリ領域の malloc に失敗する  
	XC を利用する場合、デフォルト状態の確保可能メモリサイズは 64K バイトです。
	これをハードウェア上確保可能な最大サイズに拡張するには、allmem() を実行します。
...
* c++ の対応は限定的  
	デフォルトコンストラクタ、デフォルトデストラクタの実行には未対応です。
